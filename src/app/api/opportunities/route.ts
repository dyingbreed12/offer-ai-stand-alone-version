  ...
  try {
    ...
    const res = await fetch(
      `https://services.leadconnectorhq.com/opportunities/${opportunityId}`,
      {
        ...
        // The locationId property has been removed from the body
        body: JSON.stringify({
          customFields: [
            {
              id: customFieldId,
              field_value: field_value, // ✅ CORRECTED: Changed 'fieldValue' to 'field_value'
            },
          ],
        }),
      }
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || "Failed to update opportunity." },
        { status: res.status }
      );
    }
